<template>
	<div>
		...
		<b-card id="update-product" class="main-card search-wrapper mb-20">
					...
					<b-col xl="4">
						<div v-if="currentData" class="banner-image__wrapper">
							<b-form-group>
								...
								<div class="custom-banner-image banner-image__preview mb-2" :style="currentData.image_link_thumbnail
									? { 'background-image': `url(${currentData.image_link_thumbnail})` }
									: null
								">
									<div class="overlay" v-if="currentData.image_link_thumbnail">
										<button class="banner-image__btn" @click="showImageSelect(currentData.image_link_thumbnail)">
											<i class="fas fa-eye"></i>
										</button>
										<button class="banner-image__btn" @click="removeImageSelect('image_link_thumbnail')">
											<i class="fas fa-trash-alt"></i>
										</button>
									</div>
								</div>
								<b-form-group>
									<b-form-input id="input-thumbnail" type="text" v-model="currentData.image_link_thumbnail" placeholder="Nhập link ảnh thumbnail">
									</b-form-input>
								</b-form-group>
							</b-form-group>
						</div>
            <div v-if="currentData" class="banner-image__wrapper">
							<b-form-group>
								...
								<div class="custom-banner-image banner-image__preview mb-2" :style="currentData.image_link_detail
									? { 'background-image': `url(${currentData.image_link_detail})` }
									: null
								">
									<div class="overlay" v-if="currentData.image_link_detail">
										<button class="banner-image__btn" @click="showImageSelect(currentData.image_link_detail)">
											<i class="fas fa-eye"></i>
										</button>
										<button class="banner-image__btn" @click="removeImageSelect('image_link_detail')">
											<i class="fas fa-trash-alt"></i>
										</button>
									</div>
								</div>
								<b-form-group>
									<b-form-input id="input-img-detail" type="text" v-model="currentData.image_link_detail" placeholder="Nhập link ảnh">
									</b-form-input>
								</b-form-group>
							</b-form-group>
						</div>
                  ...
                  >
                    Số thứ tự nhỏ hơn 100.
							...
							<b-row>
								<b-col md="4">
									<b-form-group>
										<label>Tiêu đề bài đăng <span class="text-danger">*</span>:</label>
										<b-form-input id="input-title" v-model.trim="$v.currentData.title.$model"
											placeholder="Nhập tiêu đề bài đăng" :class="{
												'is-invalid': validationStatus($v.currentData.title),
											}"></b-form-input>
										<div v-if="!$v.currentData.title.required" class="invalid-feedback">
								...
								<b-col md="4">
									<b-form-group>
										...
										<multiselect v-model="$v.currentData.user.$model" track-by="text" label="text"
											:show-labels="false" placeholder="Chọn"
				...
				<div class="w-100">
					...
					<div v-if="validationStatus($v.currentData.content)" class="invalid-editor">
						<div v-if="!$v.currentData.content.required" class="invalid-editor-feedback">
		...
		<b-modal id="banner-image" title="Ảnh bài đăng" :no-close-on-backdrop="true" size="lg" hide-footer>
			<div class="d-flex justify-content-center" v-if="currentUrlImage">
				<img style="width: 40rem" class="custom-banner-image" :src="currentUrlImage" alt="Ảnh banner" />
			</div>
		</b-modal>
	</div>
</template>
...
export default {
	...
	methods: {
      ...
      this.currentUrlImage = value
		...
		handleCreateAndUpdatePost() {
          ...
          image_link_thumbnail,
			...
			this.$store.dispatch(this.isUpdate ? UPDATE_POST : CREATE_POST, this.isUpdate ? payload : payload.postData).then((res) => {
				...
				let successMsg = `${this.isUpdate ? 'Cập nhật' : 'Tạo'} bài đăng thành công.`
				let errorMsg = `${this.isUpdate ? 'Cập nhật' : 'Tạo'} bài đăng không thành công.`
				if (res.status === 200) {
					this.$message({
						message: successMsg,
						type: "success",
		...
		handleReset() {
			this.currentData = this.isUpdate ? Object.assign({}, this.saveCurrentData) : Object.assign({}, initPost)

<template>
  <div>
        ...
        <b-table :items="getOrders" :fields="visibleFields" :bordered="true" :hover="true" :fixed="true"
          ...
          <template #cell(orderStatus)="row">
            <div v-if="row.item.orderStatus" class="d-flex justify-content-center">
              <b-badge v-if="row.item.orderStatus.id === 1" class="badge-process custom-badge-label">{{
                  row.item.orderStatus.statusName
              }}</b-badge>
              <b-badge v-if="row.item.orderStatus.id === 2" class="badge-success custom-badge-label">{{
                  row.item.orderStatus.statusName
              }}</b-badge>
              <b-badge v-else-if="row.item.orderStatus.id === 3" class="badge-inactive custom-badge-label">{{
                  row.item.orderStatus.statusName
              }}</b-badge>
            </div>
          </template>
          ...
          <template #cell(actions)="row">
            <div class="d-flex justify-content-center flex-wrap">
              <div>
                <a v-if="row.item.orderStatus && checkShowAction(ACTION_FOR_ORDER.UPDATE, row.item.orderStatus.id)"
                  href="javascript:void(0)" type="button" v-b-tooltip.hover title="Cập nhật" class="m-2"
                  @click.prevent="navigateToUpdateOrder(row.item)">
                  <i class="fas fa-edit" style="font-size: 1rem"></i>
                </a>
                <a v-if="row.item.orderStatus && checkShowAction(ACTION_FOR_ORDER.DELETE, row.item.orderStatus.id)"
                  href="javascript:void(0)" type="button" v-b-tooltip.hover title="Xoá" class="m-2"
                  @click.prevent="openModalActionForOrder(row.item, ACTION_FOR_ORDER.DELETE)">
                  <i class="fas fa-trash" style="font-size: 1rem; color: red"></i>
                </a>
              </div>
              <div>
                <a v-if="row.item.orderStatus && checkShowAction(ACTION_FOR_ORDER.CONFIRM, row.item.orderStatus.id)"
                  href="javascript:void(0)" type="button" v-b-tooltip.hover title="Duyệt" class="m-2"
                  @click.prevent="openModalActionForOrder(row.item, ACTION_FOR_ORDER.CONFIRM)">
                  <i class="fas fa-check" style="font-size: 1rem"></i>
                </a>
                <a v-if="row.item.orderStatus && checkShowAction(ACTION_FOR_ORDER.REJECT, row.item.orderStatus.id)"
                  href="javascript:void(0)" type="button" v-b-tooltip.hover title="Huỷ" class="m-2"
                  @click.prevent="openModalActionForOrder(row.item, ACTION_FOR_ORDER.REJECT)">
                  <i class="fas fa-times" style="font-size: 1rem; color: red"></i>
                </a>
              </div>
            </div>
          </template>
    ...
    <b-modal hide-footer id="confirm-action-for-order" :title="currentTitleModalAction ? currentTitleModalAction : 'Xác nhận thao tác với đơn hàng'" :no-close-on-backdrop="true">
      <div class="pb-3">
        <span>{{ currentContentModalAction ? currentContentModalAction : '' }}</span> <span class="font-weight-bold" v-if="this.currentOrder">{{
          this.currentOrder.orderId
          }}</span> không ?
      </div>
      <b-button class="mr-2 btn-light2 pull-right" @click="cancelActionForOrder()">
        Hủy
      </b-button>
      <b-button variant="primary pull-right" class="mr-2" type="submit" @click="handleActionForOrder()">
        Đồng ý
      </b-button>
    </b-modal>
    <ModalCreateOrder
...
export default {
  ...
  data() {
    return {
      ...
      fields: [
        ...
        { key: "date", label: "Ngày tạo", visible: true, thStyle: { width: '6%' }, thClass: 'text-left align-middle' },
        {
          key: "actions",
          label: "Chức năng",
          visible: true,
  ...
  methods: {
    ...
    checkShowAction(action, status) {
      ...
      if (action === ACTION_FOR_ORDER.DELETE) return status === 1 || status === 3
    ...
    handleActionForOrder() {
      if (this.currentActionForOrder === ACTION_FOR_ORDER.DELETE) this.handleDeleteOrder()
      if (this.currentActionForOrder === ACTION_FOR_ORDER.CONFIRM || this.currentActionForOrder === ACTION_FOR_ORDER.REJECT) this.handleConfirmAndRejectOrder()
    },
    async handleConfirmAndRejectOrder() {
      ...
      let { totalPrice, note, orderStatus, date, promotion, address, city, district, wards, phoneNumber } = { ...this.currentOrder }

      let payload = {
        totalPrice: totalPrice + '', 
        phoneNumber: phoneNumber ? Number(phoneNumber) : '',
        orderStatusId: this.currentActionForOrder === ACTION_FOR_ORDER.CONFIRM ? '2' : this.currentActionForOrder === ACTION_FOR_ORDER.REJECT ? '3' : orderStatus.id + '',
        promotionId: promotion ? promotion.promotionId : null,
        address, city, district, wards, phoneNumber, note, date,
      }

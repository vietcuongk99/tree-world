<template>
    <b-modal size="xl" hide-footer id="modal-order-detail" :title="currentTitleModal" :no-close-on-backdrop="true"
        ...
        <div class="mb-4">
          ...
          <div class="checkout__order">
                ...
                <div class="checkout__order__total">
                  Tổng giá đơn hàng <span>{{ getFormatPrice(getTotalProductPrice) }}đ</span>
                </div>
            </div>
        </div>
      </b-card>
    ...
    computed: {
      ...
      getTotalProductPrice() {
        return this.currentOrderDetail && this.currentOrderDetail.length > 0
          ? this.currentOrderDetail.map(item => item.productPrice  ? item.productPrice : 0).reduce((prev, current) => prev + current, 0)
          : 0
      },
    },
    methods: {
